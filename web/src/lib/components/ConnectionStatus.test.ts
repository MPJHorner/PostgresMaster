...
describe('ConnectionStatus Component', () => {
	...
	describe('Badge variant logic', () => {
		it('should return correct badge variant for CONNECTED state', () => {
			...
			const variant = ConnectionState.CONNECTED === ConnectionState.CONNECTED ? 'default' : 'outline';
		...
		it('should return correct badge variant for CONNECTING state', () => {
			const variant = ConnectionState.CONNECTING === ConnectionState.CONNECTING ? 'secondary' : 'outline';
			expect(variant).toBe('secondary');
		});

		it('should return correct badge variant for DISCONNECTED state', () => {
			const variant = ConnectionState.DISCONNECTED === ConnectionState.DISCONNECTED ? 'outline' : 'default';
	...
	describe('Status text logic', () => {
		it('should return correct status text for CONNECTED state', () => {
			const statusText = ConnectionState.CONNECTED === ConnectionState.CONNECTED ? '● Connected' : '';
			expect(statusText).toBe('● Connected');
		});

		it('should return correct status text for CONNECTING state', () => {
			const statusText = ConnectionState.CONNECTING === ConnectionState.CONNECTING ? '● Connecting...' : '';
			expect(statusText).toBe('● Connecting...');
		});

		it('should return correct status text for RECONNECTING state', () => {
			const statusText = ConnectionState.RECONNECTING === ConnectionState.RECONNECTING ? '● Reconnecting...' : '';
			expect(statusText).toBe('● Reconnecting...');
		});

		it('should return correct status text for ERROR state', () => {
			const statusText = ConnectionState.ERROR === ConnectionState.ERROR ? '● Connection Error' : '';
			expect(statusText).toBe('● Connection Error');
		});

		it('should return correct status text for DISCONNECTED state', () => {
			const statusText = ConnectionState.DISCONNECTED === ConnectionState.DISCONNECTED ? '● Disconnected' : '';
	...
	describe('Custom badge class logic', () => {
		it('should return green classes for CONNECTED state', () => {
			const customClass = ConnectionState.CONNECTED === ConnectionState.CONNECTED
		...
		it('should return blue classes for CONNECTING state', () => {
			const customClass = ConnectionState.CONNECTING === ConnectionState.CONNECTING
		...
		it('should return yellow classes for RECONNECTING state', () => {
			const customClass = ConnectionState.RECONNECTING === ConnectionState.RECONNECTING
		...
		it('should return gray classes for DISCONNECTED state', () => {
			const customClass = ConnectionState.DISCONNECTED === ConnectionState.DISCONNECTED
